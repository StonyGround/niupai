...
public class FragmentFollow extends BaseDynamicFragment implements SwipeRefreshLayout.OnRefreshListener {
    ...
    @Bind(R.id.follow_login)
    TextView followLogin;
    @Bind(R.id.ll_follow_login)
    LinearLayout llFollowLogin;
    @Bind(R.id.follow_nothing)
    TextView followNothing;
    ...
    private Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case Contact.GET_FOLLOW_DATA:
                    String jsonFollow = msg.obj.toString();
                    setFollowData(AESUtil.decode(jsonFollow));
                    break;
    ...
    private void setFollowData(String json) {
        Gson gson = new Gson();
    ...
    @OnClick(R.id.follow_login)
    public void onViewClicked() {
        if (CacheUtils.getInt(getActivity(), Contact.USERIDX) == 0) {
            isFromLogin = true;
            startActivity(new Intent(getActivity(), QuickLoignActivity.class));
        }
    }
...
//        recyclerview.addItemDecoration(new SpaceItemDecoration(5));
    ...
    private void getFollowData(int begin) {
        ...
        Request.Builder requestBuilder = new Request.Builder().url(Contact.HOST + Contact
                .INDEX + "?type=3&uidx=" + CacheUtils.getInt(getActivity(), Contact.USERIDX) +
                "&begin=" + begin + "&num=" + Contact.HOME_PAGE_NUM + "&vid=" + minVid);
        ...
        call.enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                message.what = Contact.GET_FOLLOW_DATA;
                handler.sendMessage(message);
            }
        });
    }
    ...
    private void setBannerData(String json) {
        ...
        adapterHomeBanner = new AdapterHomeBanner(getActivity(), bannerList);
        viewpager_banner.setAdapter(adapterHomeBanner);
        viewpager_banner.setCurrentItem(currentItem);
...
//
    ...
    public void onResume() {
        ...
        if (CacheUtils.getInt(getActivity(), Contact.USERIDX) != 0) {
            llFollowLogin.setVisibility(View.GONE);
            if (isFromLogin) {
                isFromLogin = false;
                getFollowData(0);
            }
            ...
            llFollowLogin.setVisibility(View.VISIBLE);
            swipeRefresh.setEnabled(false);
            if (adapterHomeRecyler != null) {
                adapterHomeRecyler.clearDatas();
//                adapterHomeRecyler.setHeaderView(null);
            }
        }
        followNothing.setVisibility(View.GONE);
    }
