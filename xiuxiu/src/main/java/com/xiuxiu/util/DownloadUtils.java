...
public class DownloadUtils {
    ...
    private DubMusicDataInfo.ResultBean resultBeen;
    ...
    public void download(final DubMusicDataInfo.ResultBean resultBeen) {
        this.resultBeen = resultBeen;
        final String title = new String(android.util.Base64.decode(resultBeen.getTitle().getBytes(), android.util.Base64.DEFAULT));
...
//                       if (mListener != null) mListener.onFailed(title +"的MP3下载失败");
                        break;
                    case SUCCESS_MP3:
                        if (mListener != null) mListener.onDowload(title + "的MP3已经下载");
...
//                       if (mListener != null) mListener.onFailed(title +"已存在");
    ...
    //获取下载音乐的URL
    private void getDownloadMusicURL(final DubMusicDataInfo.ResultBean resultBeen, final Handler handler) {
        mThreadPool.execute(new Runnable() {
            @Override
            public void run() {
                String AudioUrl = new String(android.util.Base64.decode(resultBeen.getAudioUrl().getBytes(), android.util.Base64.DEFAULT));
...
//                    String lr = lraUrl.substring(lraUrl.lastIndexOf('.') + 1);
    ...
    //下载MP3
    private void downloadMusic(final DubMusicDataInfo.ResultBean resultBeen, final String url, final Handler handler) {
        mThreadPool.execute(new Runnable() {
            @Override
            public void run() {
                ...
                String mp3url = url;
                String Title = new String(android.util.Base64.decode(resultBeen.getTitle().getBytes(), Base64.DEFAULT));
